...
var shaderStrArray=[["a","a"],["a","a"]];
var shaderNumberCount=[0,0];
var shaderTypeName=["vertex","fragment"];
function processLoadShader(req,index,sharderindex,url)//处理着色器脚本内容的回调函数
{
    if (req.readyState == 4) //若状态为4
    {
        var shaderStr = req.responseText;//获取响应文本
        shaderStrArray[index][sharderindex]=new shaderObject(shaderTypeName[sharderindex],shaderStr);//顶点着色器脚本内容
        shaderNumberCount[index]++;
        if(shaderNumberCount[index]>1)//如果两个着色器内容均不为空
        {
            shaderProgArray[index]=loadShaderSerial(gl,shaderStrArray[index][0], shaderStrArray[index][1],index,url);//加载着色器
...
//加载着色器的方法
function loadShaderFile(url,index,sharderindex)//从服务器加载着色器脚本的函数
{
    var req = new XMLHttpRequest();//创建XMLHttpRequest对象
    req.onreadystatechange = function () //设置响应回调函数
    { processLoadShader(req,index,sharderindex,url) };//调用processLoadShader处理响应
