//加载单个着色器的方法
function loadSingleShader(ctx, shaderScript,sharderindex,url)
{
    ...
    console.log(url+"--"+shaderScript.type+"--"+compiled+"--"+sharderindex+"--"+shaderScript.text)
...
//加载顶点着色器与片元着色器的套件
function loadShaderSerial(gl, vshader, fshader,sharderindex,url)
{
    //加载顶点着色器
    var vertexShader = loadSingleShader(gl, vshader,sharderindex,url);
    //加载片元着色器
    var fragmentShader = loadSingleShader(gl, fshader,sharderindex,url);
