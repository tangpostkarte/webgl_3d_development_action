...
addBloomAndGUI=(scene,cameraArry)=>{
    ...
    let pipeline = new BABYLON.DefaultRenderingPipeline("pipeline",true, scene, cameraArry); 
    ...
    checkbox.onIsCheckedChangedObservable.add((state)=> {
        pipeline.bloomEnabled = state;
    });
    let checkboxText='是否进行Bloom渲染';
    let header = BABYLON.GUI.Control.AddHeader(checkbox, checkboxText, "280px", { isHorizontal: true, controlFirst: true});
    header.height = "30px";
    header.color = "blank";
    ...
    addSlider("内核尺寸", function(value) {
        pipeline.bloomKernel = value;
    }, pipeline.bloomKernel, 1, 500, panel);
    addSlider("宽度", function(value) {
        pipeline.bloomWeight = value;
    }, pipeline.bloomWeight, 0.0, 1.0,panel );
    addSlider("亮度阙值", function(value) {
        pipeline.bloomThreshold = value;
    }, pipeline.bloomThreshold, 0.0, 1.0,panel );
    addSlider("范围大小", function(value) {
        pipeline.bloomScale = value;
    }, pipeline.bloomScale, 0.1, 1.0,panel );   
