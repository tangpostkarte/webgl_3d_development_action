...
const addMesh=(scene,directionLight)=>{
    ...
    let  kjzTask = loader.addMeshTask("ch_gltf", "","./model/", "ch.STL");//创建加载任务
    kjzTask.onSuccess = (task)=> {//加载成功后任务
        for(let mesh of task.loadedMeshes)//遍历加载到的模型
        {
            mesh.position=new BABYLON.Vector3(0,5,0);//设置每个物体的位置
            mesh.renderingGroupId = 1;//设置渲染天空盒在其他物体渲染之后
    ...
    setTimeout(()=>{
        console.log(`meshArray${meshArray.length}|||${meshArray[0].backFaceCulling}`);
        var mat = new BABYLON.StandardMaterial("ch_mat", scene);
        mat.backFaceCulling=false;
        for(let tempMesh of meshArray)
        {
            tempMesh.receiveShadows=true;
            tempMesh.material=mat;
            //tempMesh.backFaceCulling=false;
